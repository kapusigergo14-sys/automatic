/**
 * SEND HVAC — chatbot pitch to modern HVAC service sites.
 *
 * Pipeline:
 * 1. Load qualified hvac leads from hvac-modern.json
 * 2. Load send-state for dedup
 * 3. For each lead (rate-limited):
 *    - Rotate subject (3 variants)
 *    - Fill body template with {Company}
 *    - Send via Brevo API
 *    - Append to send-state-hvac.json
 *
 * CLI:
 *   npx ts-node scripts/send-hvac.ts [--limit 30] [--dry-run]
 */
...
import { pickSubject, buildBody } from './email-bodies-hvac';
...
const LEADS_FILE = path.resolve(__dirname, '../output/leads/hvac-modern.json');
const V5_STATE_FILE = path.resolve(__dirname, '../output/v5-campaign/send-state-hvac.json');
...
async function commitAndPushState(email: string): Promise<void> {
  const relState = 'output/v5-campaign/send-state-hvac.json';
  try {
    ...
    execSync(
      `git -c commit.gpgsign=false commit -m "hvac send: ${email}" --no-verify`,
      { stdio: 'pipe' }
    );
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║  HVAC CHATBOT SEND — HVAC service pitch                       ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝');
  ...
  if (!fs.existsSync(LEADS_FILE)) {
    console.log('No leads file. Run `osm-collect.ts --industry hvac` first.');
    return;
  }
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║  HVAC CHATBOT SEND COMPLETE                                   ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝');

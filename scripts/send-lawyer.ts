/**
 * SEND LAWYER — chatbot pitch to modern law firm sites.
 *
 * Pipeline:
 * 1. Load qualified lawyer leads from lawyer-modern.json
 * 2. Load send-state for dedup
 * 3. For each lead (rate-limited):
 *    - Rotate subject (3 variants)
 *    - Fill body template with {Company}
 *    - Send via Brevo API
 *    - Append to send-state-lawyer.json
 *
 * CLI:
 *   npx ts-node scripts/send-lawyer.ts [--limit 30] [--dry-run]
 */
...
import { pickSubject, buildBody } from './email-bodies-lawyer';
...
const LEADS_FILE = path.resolve(__dirname, '../output/leads/lawyer-modern.json');
const V5_STATE_FILE = path.resolve(__dirname, '../output/v5-campaign/send-state-lawyer.json');
...
async function commitAndPushState(email: string): Promise<void> {
  const relState = 'output/v5-campaign/send-state-lawyer.json';
  try {
    ...
    execSync(
      `git -c commit.gpgsign=false commit -m "lawyer send: ${email}" --no-verify`,
      { stdio: 'pipe' }
    );
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║  LAWYER CHATBOT SEND — law firm pitch (multi-market)          ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝');
  ...
  if (!fs.existsSync(LEADS_FILE)) {
    console.log('No leads file. Run `osm-collect.ts --industry lawyer` first.');
    return;
  }
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║  LAWYER CHATBOT SEND COMPLETE                                  ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝');
